...
static enum cb_err parse_ec_version(const char *str, uint32_t *version)
{
	 ...
	 * `_` separator.
	 *
	 * Returned integer version: 0xYYYYMMDD
	 */
...
static void add_ec_fw_info(struct lb_header *header)
{
	...
	if (strcmp(CONFIG_DRIVERS_EFI_EC_FW_GUID, "") == 0) {
		/* No EC or its updates aren't done using capsules. */
		return;
	}

	if (parse_uuid(guid, CONFIG_DRIVERS_EFI_EC_FW_GUID)) {
		printk(BIOS_WARNING, "%s(): failed to parse EC firmware's GUID: '%s'\n",
		       __func__, CONFIG_DRIVERS_EFI_EC_FW_GUID);
		return;
	}
	...
	fw_info->lowest_supported_version = CONFIG_DRIVERS_EFI_EC_FW_LSV;
	fw_info->fw_size = CONFIG_DRIVERS_EFI_EC_FW_SIZE;
}
